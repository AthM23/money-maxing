...
OUR does not fully prevent them. Charge option is MT103 **71A** (pacs.008 `ChrgBr`); deductions appear in **71F/71G**.
Dr 6150 / Cr 1200. **Arithmetic** (invoice − deposit − FX = fee) once the advice states it; **judgment** only on *who
bears it*, a contract term. Evidence: credit advice, 71F, camt.053 `Chrgs`.
...
([HighRadius](https://www.highradius.com/resources/Blog/cash-application/)). **Judgment** until a document says
otherwise, then arithmetic.
...
rate until cash-settled or remeasured at period end (IAS 21, above). Dr 1000 + Dr/Cr 7100 on the settled slice only.
**Arithmetic**; the *reason* for the remainder is judgment.
...
risk ([Maximor](https://www.maximor.ai/blog/indirect-tax-usage-based-billing)). Dr 1350 / Cr 1200. **Judgment** until a
certificate or authorised answer exists (tax vs dispute vs discount); **arithmetic** once the rate is known. Brazil
and Indonesia deduct similarly — unverified, name no rates.
...
(a) a third-party payer — the kernel refuses the party tie, needs an approved-payer fact; (b) cash into the *wrong
seller entity's* account — Dr 1000 (B) / Cr **2900 Due to A**, mirrored Dr 1500 Due from B / Cr 1200 in A, eliminated
on consolidation. Kiteworks is the shape: 10 entities, 7 currencies, 66 accounts, agents "translate currencies and
eliminate intercompany transactions" ([Maximor](https://www.maximor.ai/case-study/kiteworks)). **Judgment** on whether
this payer may pay for that customer; arithmetic thereafter.
...
mirror image: the seller *did* charge VAT (VAT ID invalid) and the buyer pays net of it — a **billing error, not a
deduction**; credit note and reissue, never a write-off. **Judgment**; evidence is the validated VAT ID and the
invoice's Art. 226(11a) mention.
...
**Arithmetic.** Lockboxes widen the gap. Inference: a rec item, not a match failure, hence ranked low.
...
**14. Duplicates and overpayments.** Unapplied cash, not revenue and not a discount
...
Per **receipt**: bank account and owning entity; **value and booking date** (camt.053/BAI2/MT940); foreign amount
...
certificate reference; trace id of every document quoted.

Reality check: rate and fee exist **only** in the credit advice, not the statement feed; the deduction reason **only**
in a remittance advice or email; the payer's authority **only** in a contract or inbox. Hence "read the document,
re-perform the arithmetic".
...
credit or billing error; whether a payer may pay for a customer; the authoritative rate source and rounding tolerance;
disposition of overpayments; anything under a compliance hold. Scope every answer by party, ceiling and end date, as
`GATE1_ANSWERS_A.md` does with the 2% SLA rate.

**The honest line:** a withheld amount is a tax when a certificate or an authorised person says so, not because the
remittance says "TDS". Until then, book to 1350 and keep the evidence gap open.
...
residual = invoice. Show both rates and their sources beside the entry. Don't claim multi-currency books.

**2. Split fee from FX, labelling charge option SHA on the advice.** Check the fee equals the advice's stated
deduction, not a plug. Show three causes, three entries, one wire. Don't claim you predict the fee.
...
**booked** rate. Show the two-period timeline. Don't claim CTA.

**4. Wrong-entity receipt → intercompany due-to/due-from.** Seed a wire into the Singapore account for a UK invoice.
Check both legs post, the pair eliminates, AR control ties. Show the cash-by-entity strip move. Don't claim a
consolidation engine.

**5. Withholding held as a receivable, certificate missing.** Seed "NET OF TDS", no Form 16A. Check the 1350 balance
plus an evidence gap that closes when the certificate lands. Show the gap as a *balance*. Don't claim treaty-rate
determination.

## 5. Mistakes a finance judge would catch

FX to misc expense rather than a named FX gain/loss line. Realizing FX on the unpaid remainder. The invoice-date rate
...
applied with no approved-payer record. An overpayment cleared to revenue. A "cash view by currency and entity" built
on all-USD receipts.
