...
- [ ] Repo scaffold per spec §6 (TypeScript, Node 22, pnpm, better-sqlite3, zod, vitest). Integer cents everywhere.
- [ ] `src/contract/schema.sql` from spec §7, plus a yes/no on each addition the board found
...
- [ ] `Proposal`, `Mark`, tool names from spec §8, event topics from
      [`EVENT_TOPICS.md`](./diagrams/architecture/EVENT_TOPICS.md) as constants.
- [ ] Decide and log in `PROJECT_STATUS.md` (each takes two minutes and blocks real code):
