...
function adviceProblems(proposal: Proposal, ctx: KernelContext, adviceId: string | null, foreignCents: number, ratePpm: number): string[] {
  ...
  if (!trace.payload_text.includes(amountText(foreignCents))) problems.push(`bank advice ${adviceId} does not state ${amountText(foreignCents)}`);
  if (!trace.payload_text.includes(rateText(ratePpm))) problems.push(`bank advice ${adviceId} does not state the rate ${rateText(ratePpm)}`);
  return problems;
}
