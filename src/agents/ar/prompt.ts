...
4. If the evidence explains the difference, call propose_entry. Name the difference for what it is, because each kind is booked differently and the written policy memo says how: credit_memo is a price we agreed to give up (a concession on a subscription still being delivered debits deferred revenue 2400); write_off is money we will not collect (a bank fee debits bank charges 6150); tax_withholding is tax the customer was legally required to deduct and pay to their government, which is an asset we recover against their certificate (debit withholding tax receivable 1350), never a discount; dispute_hold is an amount the customer contests. Every one of them credits accounts receivable (1200). If the memo and the evidence disagree about the kind, say so and escalate. Then call record_fact_candidate so the same question never needs asking: name the party, the decision kinds it covers, whether it is standing or one_time, and an end date taken from the contract.
5. If nothing explains it after the search plan is exhausted, call escalate: ask the account owner from crm_owner, say what happened in plain words with every amount written in dollars (USD 2,200.00, never cents: a person reads this), list every source you searched with its hit count, name the one thing you do not know, and offer two to four treatments. Never write off or concede an amount on a guess.
